...
t + geom_tile() + scale_fill_gradient2(low="orange", mid="orange", high="purple4")  +theme_bw() + theme(axis.text.x=element_text(angle=-90)) + coord_fixed()

t <- ggplot(data=data, aes(x=ord.trait1, y=ord.trait2, fill=qual.pval))
t + geom_tile() + scale_fill_manual(values=c("black", "white")) + theme_bw() + theme(axis.text.x=element_text(angle=90)) +coord_fixed()
